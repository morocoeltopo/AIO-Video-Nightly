...
object VideoFormatsUtils {
    ...
    private fun formatDownloadLine(input: String): String {
        val regex =
            Regex("""(\d+\.\d+%) of ~?\s+([\d.]+[KMGT]iB) at\s+([\d.]+[KMGT]iB/s) ETA\s+([\d:]+) \(frag (\d+/\d+)\)""")
    ...
    private fun formatDownloadLineStage2(input: String): String {
        val regex =
            Regex("""(\d+\.\d+%) of ~?\s+([\d.]+[KMGT]iB) at\s+([\d.]+[KMGT]iB/s) ETA\s+([:\-\w]+) \(frag (\d+)/(\d+)\)""")
        val matchResult = regex.find(input)
    ...
    private fun formatDownloadLineStage3(input: String): String {
        val regex =
            Regex("""(\d+\.\d+%) of\s+([\d.]+[KMGT]iB) at\s+([\d.]+[KMGT]iB/s) ETA\s+([\d:]+)""")
        val matchResult = regex.find(input)
        return if (matchResult != null) {
